...
def _timing_finding(unit: TranslatableUnit, event: pysubs2.SSAEvent) -> Finding | None:
    if (
...
def _missing_id_finding(unit: TranslatableUnit) -> Finding:
    return Finding(
        id=unit.id,
        kind="missing_id",
...
def _pair_sequential(
    units: list[TranslatableUnit], events: list[pysubs2.SSAEvent]
) -> tuple[list[ReviewLine], list[Finding]]:
    """Pair by position (units[i] <-> events[i]); used for re-segmented SRT targets."""
    lines: list[ReviewLine] = []
    findings: list[Finding] = []
    for pos, unit in enumerate(units):
        if pos >= len(events):
            findings.append(_missing_id_finding(unit))
            continue
        event = events[pos]
        lines.append(
            ReviewLine(
        ...
        timing = _timing_finding(unit, event)
...
def _pair_by_index(
    ...
    source_subs: pysubs2.SSAFile | None,
    compare_styles: bool,
) -> tuple[list[ReviewLine], list[Finding]]:
    """Pair by unit.event_index; used for ASS targets where preserved events keep the mapping."""
...
def _global_findings(
    ...
    """Whole-file checks: duplicate stable IDs and chronological order of the target."""
    findings: list[Finding] = []
...
def pair_lines(
    ...
    *,
    ...
    events = target_subs.events
    if sequential:
        lines, findings = _pair_sequential(units, events)
    else:
        lines, findings = _pair_by_index(units, target_subs, source_subs, compare_styles)
    findings.extend(_global_findings(units, events))
    return lines, findings
